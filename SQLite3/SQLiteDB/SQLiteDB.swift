...
class SQLiteDB {
    ...
    //是否存在数据
    func isExists(tableName: String, condition: String) -> Bool {
        let sql = "SELECT * FROM \(tableName) WHERE \(condition)"
        let data = self.query(sql)
        if data.count > 0 {
            return true
        }
        return false
    ...
    //查询数据
    func query(tableName: String, condition: String)->[SQLRow] {
        let sql = "SELECT * FROM \(tableName) WHERE \(condition)"
        return query(sql)
    }
    
    //删除数据
    func delete(tableName: String, condition: String? = nil) {
        var sql: String!
        if condition != nil {
    ...
    // 改变数据
    func updata(tableName: String, updateContent: String, condition: String) {
        var sql: String!
        sql = "update \(tableName) set \(updateContent) where \(condition)"
        execute(sql)
    }
}

//获得字段数据
func getValue(row: SQLRow, field: String) -> SQLColumn? {
    if let column = row[field] {
        return column
    }else {
        return nil
    }
